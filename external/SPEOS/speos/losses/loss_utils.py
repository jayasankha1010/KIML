...
def dcg(y_pred, y_true, ats=None, gain_function=lambda x: torch.pow(2, x) - 1, padding_indicator=-1):
    ...
    discounts = (torch.tensor(1) / torch.log2(torch.arange(true_sorted_by_preds.shape[1], dtype=torch.double) + 2.0)).to(
...
def mrr(y_pred, y_true, ats=None, padding_indicator=-1):
    ...
    within_at_mask = (indices < ats_rep).type(torch.double)
...
def sinkhorn_scaling(mat, mask=None, tol=1e-6, max_iter=50):
    ...
    check https://openreview.net/pdf?id=uR77O7SL55h for more efficient implementation
    """
    ...
    for k in range(max_iter):
        # print("Before data copying: CUDA Memory allocated: {}; reserved: {}".format(
        ...
        # print("After data copying: CUDA Memory allocated: {}; reserved: {}".format(
        #    torch.cuda.memory_allocated(), torch.cuda.memory_reserved()))
...
def deterministic_neural_sort(s, tau, mask):
    ...
    n = s.size()[1]
    one = torch.ones((n, 1), dtype=torch.double, device=dev)
    ...
    temp = [t.type(torch.double) for t in temp]
    temp = [torch.cat((t, torch.zeros(n - len(t), device=dev))) for t in temp]
    scaling = torch.stack(temp).type(torch.double).to(dev)  # type: ignore
